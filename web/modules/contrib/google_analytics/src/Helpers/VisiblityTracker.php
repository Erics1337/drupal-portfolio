...
class VisiblityTracker {
  ...
  public function getVisibilityRoles($account) {
    ...
    // Ensure $visibility_user_role_roles is an array.
    $visibility_user_role_roles = $this->config->get('visibility.user_role_roles') ?? [];
    if (!is_array($visibility_user_role_roles)) {
      $visibility_user_role_roles = [];
    }
  
    // Check if roles array is not empty before iterating.
    if (is_array($visibility_user_role_roles) && count($visibility_user_role_roles) > 0) {
